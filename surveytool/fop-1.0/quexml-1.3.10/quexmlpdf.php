 ...
 */
require_once(ROOT . '/fop-1.0/quexml-1.3.10/tcpdf/config/lang/eng.php');
...
class queXMLPDF extends TCPDF {
	 ...
	 * @since 2010-10-29
	 */
	protected $questionnaireInfoMargin = 5; //MBK6WM - change from 20 to 5
	 ...
	 * @since 2010-11-05
	 */
	protected $responseLabelHeight = 5;
	 ...
	 * @since 2010-11-05
	 */
	protected $sectionHeight = 9; //JAH5FV modified 1/23/13
	...
	public function createqueXML($quexml)
	{
		...
		foreach($xml->section as $s)
		{
			...
			$stmp['title'] = ""; //MBK6WM - change from "Section " . $sl to ""
			$stmp['info'] = "";
			$stmp['text'] = "";
			...
			foreach ($s->question as $qu)
			{
				...
				$qtmp['title'] = "" . $qcount . $this->questionTitleSuffix; //MBK6WM - change $sl to ""
				$qtmp['text'] = "";
	...
	protected function addSection($desc = 'queXMLPDF Section',$title = false,$info = false)
	{
		...
		$html = "<span class=\"sectionTitle\">$title</span>&nbsp;<span class=\"sectionDescription\">$desc</span>"; //MBK6WM - change $title: to $title
